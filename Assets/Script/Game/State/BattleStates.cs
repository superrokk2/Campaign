...
{
    /// <summary>모든 전투 단계가 따르는 공통 생명주기 계약입니다.</summary>
    ...
    /// <summary>전투를 멈춘 채 시작 입력을 기다리고 준비 HUD를 표시합니다.</summary>
    ...
    /// <summary>
    /// 경과 시간, HUD 갱신 주기, 종료 조건과 승패 판정을 소유하는 핵심 State입니다.
    /// 이 책임을 Flow에서 분리해 새 상태를 추가할 때 Controller가 비대해지는 것을 막습니다.
    /// </summary>
    public sealed class BattleState : IBattleState
    {
        ...
        public void Tick(float deltaTime)
        {
            ...
            // 모든 유닛의 Tick을 한 번에 호출해 실행 순서를 예측 가능하게 합니다.
            flow.TickCombat(deltaTime);

            // 문자열 생성과 Canvas rebuild를 매 프레임 하지 않도록 주기 또는 인원 변화 시에만 갱신합니다.
            ...
            // 양 팀이 생존하고 제한 시간이 남아 있으면 현재 State를 유지합니다.
    ...
    /// <summary>전투 Tick을 중단하고 결과와 재시작 입력만 노출합니다.</summary>
