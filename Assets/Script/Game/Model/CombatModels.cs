...
{
    // Model 계층은 UnityEngine에 의존하지 않습니다. 따라서 전투 규칙을 씬이나
    // MonoBehaviour 없이 단위 테스트할 수 있고, View 교체에도 영향을 받지 않습니다.
    public enum Team { Player, Enemy }
    public enum BattlePhase { Prepare, Battle, Result }

    /// <summary>
    /// 체력의 유효 범위와 사망 전이를 소유하는 순수 C# 모델입니다.
    /// Controller는 수치를 직접 변경하지 않고 반드시 이 API를 통해 피해를 적용합니다.
    /// </summary>
    public sealed class HealthModel
    {
        ...
        {
            // 잘못된 데이터가 들어와도 체력이 0인 상태로 생성되지 않도록 방어합니다.
        ...
        {
            // 사망 후 중복 보상이나 연출이 발생하지 않도록 추가 피해를 무시합니다.
    ...
    /// <summary>시간 누적과 사용 가능 여부만 담당하는 재사용 가능한 쿨다운 모델입니다.</summary>
    ...
    /// <summary>전투원 한 명의 런타임 규칙 데이터입니다. 이동과 렌더링 구현은 포함하지 않습니다.</summary>
